...
class PlayerController : Component(), KoinComponent {
    ...
    fun isPushing(): Boolean {
        ...
        inputProvider.getAxes(GLFW.GLFW_JOYSTICK_1)?.let { axes ->
            if (axes.size > GamepadConstants.AXIS_LEFT_Y) {
                val stickY = -axes[GamepadConstants.AXIS_LEFT_Y]
                if (stickY > 0.1f) {
                    multiplier = max(multiplier, stickY)
                }
            }
            if (axes.size > GamepadConstants.AXIS_RIGHT_TRIGGER) {
                val rt = (axes[GamepadConstants.AXIS_RIGHT_TRIGGER] + 1f) / 2f
                if (rt > 0.1f) {
    ...
    fun updateProceduralLean(dt: Float) {
        ...
        var steerInput = 0f
        inputProvider.getAxes(GLFW.GLFW_JOYSTICK_1)?.let { axes ->
            if (axes.size > GamepadConstants.AXIS_LEFT_X) {
                steerInput = axes[GamepadConstants.AXIS_LEFT_X]
            }
        }
    ...
    fun handleWalking(dt: Float) {
        ...
        val moveInput = org.joml.Vector3f(moveX, 0f, moveZ)
        ...
        target.getComponent<Transform>()?.let{ transform ->
            if (moveInput.length() > 0.1f) {
                ...
                val camForward = org.joml.Vector3f(0f, 0f, -1f)
                viewInv.transformDirection(camForward)
                camForward.y = 0f
                camForward.normalize()

                val camRight = org.joml.Vector3f(1f, 0f, 0f)
                viewInv.transformDirection(camRight)
                camRight.y = 0f
                camRight.normalize()

                val moveDir = org.joml.Vector3f()
                camForward.mul(moveInput.z, moveDir)
                val rightPart = org.joml.Vector3f(camRight).mul(moveInput.x)
                moveDir.add(rightPart)

                // Apply movement to transform
                val velocity = org.joml.Vector3f(moveDir).mul(walkSpeed * dt)
        ...
        // Jump Button A
        if (inputProvider.buttonBeginPress(
                GLFW.GLFW_JOYSTICK_1,
                GamepadConstants.BUTTON_A
            ) || inputProvider.keyBeginPress(
                GLFW.GLFW_KEY_SPACE
            )
        ) {
    ...
    fun handleGroundSnapping() {
        ...
        val rayEnd = org.joml.Vector3f(pos.x, pos.y - 2f, pos.z)
    ...
    private fun bail() {
        ...
        // Inherit velocity
        val linVel = rb?.linearVelocity ?: org.joml.Vector3f()
        val angVel = rb?.angularVelocity ?: org.joml.Vector3f()
    ...
    private fun handleStateToggle() {
        var toggle = inputProvider.keyBeginPress(GLFW.GLFW_KEY_Y)
        if (inputProvider.buttonBeginPress(GLFW.GLFW_JOYSTICK_1, GamepadConstants.BUTTON_Y)) {
            toggle = true
        }
        ...
        if (toggle) {
            ...
            character.getComponent<Transform>()?.let{ transform ->
                if (stateManager?.currentState == PlayerState.RIDING) {
                    ...
                    val right = org.joml.Vector3f(0f, 0f, 0.4f)
    ...
    fun updateCurrentStance() {
        ...
        val forward = org.joml.Vector3f(1f, 0f, 0f)
    ...
    fun handleCatch(dt: Float, isGrounded: Boolean) {
        ...
        if (rollTorque != 0f || pitchTorque != 0f) {
            val localTorque = org.joml.Vector3f(rollTorque, 0f, pitchTorque)
            val worldTorque = org.joml.Vector3f()
    ...
    fun handleFlicks(dt: Float) {
        val flick = inputBuffer.getRightStickFlickVelocity(GLFW.GLFW_JOYSTICK_1, 0.1f)
        if (flick.length() > 5.0f) {
            ...
            val localTorque =
                org.joml.Vector3f(flick.y * flickSensitivity * stanceMultiplier, flick.x * flickSensitivity, 0f)
            val worldTorque = org.joml.Vector3f()
    ...
    fun handleSteering(dt: Float, isGrounded: Boolean) {
        ...
        inputProvider.getAxes(GLFW.GLFW_JOYSTICK_1)?.let { axes ->
            if (axes.size > GamepadConstants.AXIS_LEFT_X) {
                val stickX = axes[GamepadConstants.AXIS_LEFT_X]
                if (abs(stickX) > 0.1f) {
    ...
    fun handlePushing(dt: Float, isGrounded: Boolean) {
        ...
        inputProvider.getAxes(GLFW.GLFW_JOYSTICK_1)?.let { axes ->
            if (axes.size > GamepadConstants.AXIS_LEFT_Y) {
                val stickY = -axes[GamepadConstants.AXIS_LEFT_Y] // Inverted stick Y
                if (stickY > 0.1f) {
            ...
            if (axes.size > GamepadConstants.AXIS_RIGHT_TRIGGER) {
                val rt = (axes[GamepadConstants.AXIS_RIGHT_TRIGGER] + 1f) / 2f // Normalize -1..1 to 0..1
                if (rt > 0.1f) {
