...
### 6. glTF Implementation Checklist
- **Mesh & Geometry**
    - [ ] **Primitive Support**: Handle GL_TRIANGLES, GL_TRIANGLE_STRIP, and GL_TRIANGLE_FAN.
    - [ ] **Standard Attributes**: Support Position (VEC3), Normal (VEC3), and Tangent (VEC4).
    - [ ] **Texture Coordinates**: Support at least two UV sets (TEXCOORD_0, TEXCOORD_1).
    - [ ] **Vertex Coloring**: Support COLOR_0 (RGB/RGBA) multiplication in the fragment shader.
    - [ ] **Index Buffer Accessors**: Support for UNSIGNED_BYTE, UNSIGNED_SHORT, and UNSIGNED_INT.
- **PBR Material Model (Metallic-Roughness)**
    - [ ] **Base Color**: Factor (RGBA) and Texture (RGBA) with sRGB-to-Linear conversion.
    - [ ] **Metallic-Roughness**: Blue channel = Metallic, Green channel = Roughness.
    - [ ] **Normal Map**: Support for tangent-space normals with scale influence.
    - [ ] **Ambient Occlusion**: Support for the occlusionTexture (Red channel).
    - [ ] **Emissive Map**: Support for emissiveTexture and emissiveFactor.
    - [ ] **Alpha Pipeline**: Handle OPAQUE, MASK (alphaCutoff), and BLEND modes.
    - [ ] **Double Sided**: Implement the doubleSided flag by disabling backface culling per-material.
- **Transformation & Hierarchy**
    - [x] **Node Hierarchy**: Proper parent-child matrix multiplication to maintain local-to-world transforms.
    - [ ] **TRS Support**: Handle nodes defined by Translation, Rotation (Quaternions), and Scale.
    - [ ] **Coordinate System**: Convert glTF Right-Handed (Y-Up) to your engine's internal coordinate system if different.
- **Skinning & Animation**
    - [ ] **Joints & Weights**: Support for vertex skinning with up to 4 joint influences (JOINTS_0, WEIGHTS_0).
...
- **Unit Testing Framework**
    ...
    - [ ] **Math Validation**: Unit tests for JOML transformations and Trick Labeler logic.
...
## 🛹 The Board Rig Technical Checklist
