...
- AUD-007 lifecycle responsiveness follow-up: `ResourceManager.clear()` now batches GPU destruction into a single `runOnMain` task, and `DefaultJobSystem.update()` now applies a fixed per-frame main-thread task budget to prevent close/reopen task-burst frame stalls. Focused coverage added in `ResourceManagerClearBatchingTest` and `DefaultJobSystemUpdateBudgetTest`.
